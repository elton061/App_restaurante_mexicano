...
class Churros extends StatelessWidget {
  ...
  // Método para buscar os dados do Churros no Firestore
  Future<Produto> _getChurrosDetails() async {
    final FirebaseFirestore firestore = FirebaseFirestore.instance;
    try {
      // Usando a consulta para buscar pelo nome do produto
      final querySnapshot = await firestore
          .collection('itens_cardapio')
          .where('nome', isEqualTo: 'Churros') // Filtra pelo campo 'nome'
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<Produto>(
        future: _getChurrosDetails(), // Chama a função para pegar os dados do Firestore
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            ...
            return Center(child: Text('Erro ao carregar os Churros.'));
          }

          if (!snapshot.hasData) {
            print('Dados não encontrados');
            return Center(child: Text('Churros não encontrados.'));
          }

          final churros = snapshot.data!;
              ...
              children: [
                Center(
                  child: Padding(
                    ...
                    child: Image.network(
                      churros.imagem, // Usando a URL da imagem do Firestore
                    ),
                  ),
                ),
                Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Text(
                            '${churros.nome}\n${churros.descricao}', // Exibindo a descrição
                          ),
                        ),
                        Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Text(
                            'Preço: R\$ ${churros.preco.toStringAsFixed(2)}',
                            style: TextStyle(
                              fontSize: 20,
                ...
                TextButton(
                  ...
                  onPressed: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: Text('${churros.nome} adicionado ao carrinho'),
                    ));
                  },
                ),
              ],
