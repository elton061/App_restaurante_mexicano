...
class Sopaipillas extends StatelessWidget {
  Sopaipillas({super.key});

  // Método para buscar os dados do Sopaipillas no Firestore
  Future<Produto> _getSopaipillasDetails() async {
    final FirebaseFirestore firestore = FirebaseFirestore.instance;
    try {
      // Usando a consulta para buscar pelo nome do produto
      final querySnapshot = await firestore
          .collection('itens_cardapio')
          .where('nome', isEqualTo: 'Sopaipillas') // Filtra pelo campo 'nome'
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<Produto>(
        future: _getSopaipillasDetails(), // Chama a função para pegar os dados do Firestore
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            ...
            return Center(child: Text('Erro ao carregar Sopaipillas.'));
          }

          if (!snapshot.hasData) {
            print('Dados não encontrados');
            return Center(child: Text('Sopaipillas não encontrado.'));
          }

          final sopaipillas = snapshot.data!;
              ...
              children: [
                Center(
                  child: Padding(
                    ...
                    child: Image.network(
                      sopaipillas.imagem, // Usando a URL da imagem do Firestore
                    ),
                  ),
                ),
                Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Text(
                            '${sopaipillas.nome}\n${sopaipillas.descricao}', // Exibindo a descrição
                          ),
                        ),
                        Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Text(
                            'Preço: R\$ ${sopaipillas.preco.toStringAsFixed(2)}',
                            style: TextStyle(
                              fontSize: 20,
