...
class Nachos extends StatelessWidget {
  ...
  // Método para buscar os dados do nachos no Firestore
  Future<Produto> _getNachosDetails() async {
    ...
    try {
      ...
      final querySnapshot = await firestore
          ...
          .where('nome', isEqualTo: 'Nachos') // Filtra pelo nome 'Nachos Clássicos'
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<Produto>(
        future: _getNachosDetails(), // Chama a função para pegar os dados do Firestore
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return Center(child: Text('Erro ao carregar o nacho.'));
          }

          if (!snapshot.hasData) {
            return Center(child: Text('Nacho não encontrado.'));
          }

          final nachos = snapshot.data!;
              ...
              children: [
                Center(
                  child: Padding(
                    ...
                    child: Image.network(
                      nachos.imagem, // Usando a URL da imagem do Firestore
                    ),
                  ),
                ),
                Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Text(
                            '${nachos.nome}\n${nachos.descricao}', // Exibindo a descrição
                          ),
                        ),
                        Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Text(
                            'Preço: R\$ ${nachos.preco.toStringAsFixed(2)}',
                            style: TextStyle(
                              fontSize: 20,
                ...
                TextButton(
                  ...
                  onPressed: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: Text('${nachos.nome} adicionado ao carrinho'),
                    ));
                  },
                ),
              ],
